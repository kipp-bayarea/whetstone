    ...
    def GetData(self):
        ...
        try:
            ...
            response = requests.get(
                endpointurl, data=payload, headers=headers, params=params
            )
        ...
        try:
            ...
            for row in data:  # Iterate through each row in the JSON response
                ...
                for (
                    ...
                    row.keys()
                ):  # Iterate through each column in the response dictionary keys
                    if (
                        type(row[c]) is dict
                    ):  # These are the columns that can be flattened to the parent level
                    ...
                    if (
                        type(row[c]) is str
                    ):  # These are the columns with key:value pair at the parent level
                        s[c] = row[c]
